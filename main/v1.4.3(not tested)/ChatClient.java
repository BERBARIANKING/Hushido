...
public class ChatClient {
        ...
        new Thread(() -> {
            try {
                ...
                while ((incomingMessage = in.readLine()) != null) {
                    if (incomingMessage.startsWith("AVATAR:")) {
                        String[] parts = incomingMessage.split(":", 3);
                        String senderUsername = parts[1];
                        byte[] avatarData = Base64.getDecoder().decode(parts[2]);
                        gui.updateAvatar(senderUsername, avatarData);
                    } else {
    ...
    public void sendAvatar(byte[] avatarData) {
        String avatarBase64 = Base64.getEncoder().encodeToString(avatarData);
        out.println("AVATAR:" + avatarBase64);
    }
