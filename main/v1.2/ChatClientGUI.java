...
public class ChatClientGUI extends JFrame {
    ...
    private void showMainMenu() {
        ...
        // Load and play music
        loadAndPlayMusic("C:\\Users\\Berbari\\Documents\\NetBeansProjects\\ChatClient\\src\\Stryker & Sonic Species - Now Is the Time.wav"); // Add your audio file path here

        // Add a speaker button below the logo
        JPanel topPanel = new JPanel(new BorderLayout());
        topPanel.setBackground(Color.BLACK);
        JButton speakerButton = createSpeakerButton();
        topPanel.add(logo, BorderLayout.NORTH);
        topPanel.add(speakerButton, BorderLayout.SOUTH);
    ...
    private JButton createSpeakerButton() {
        JButton speakerButton = new JButton(new ImageIcon("path/to/unmute_icon.png")); // Unmute icon path
        speakerButton.setBackground(Color.BLACK);
        speakerButton.setFocusPainted(false);
        speakerButton.setBorder(BorderFactory.createEmptyBorder());
        speakerButton.addActionListener(e -> toggleMusic(speakerButton));
        return speakerButton;
    }

    private void toggleMusic(JButton speakerButton) {
        if (isPlaying) {
            audioClip.stop();
            speakerButton.setIcon(new ImageIcon("path/to/mute_icon.png")); // Mute icon path
        } else {
            audioClip.start();
            audioClip.loop(Clip.LOOP_CONTINUOUSLY);
            speakerButton.setIcon(new ImageIcon("path/to/unmute_icon.png")); // Unmute icon path
        }
        isPlaying = !isPlaying;
    }
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            if (authenticate(username, password)) {
                try {
                    ...
                    createGUI();
                    displayAsciiArt();
                    stopMusic(); // Stop the music once entering the chat
                } catch (IOException e) {
    ...
    private void connectToServer(String serverAddress, int port) throws IOException {
        client = new ChatClient(serverAddress, port, this, username, password);
    }
